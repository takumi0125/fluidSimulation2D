...
void main(){
  ...
  vec2 p = gl_FragCoord.xy - sampleVelocity(dataTex, uv, r);

  vec2 velocity = bilerpVelocity(dataTex, p, r);

  if(velocity.x == 0.0 && velocity.y == 0.0) {
    gl_FragColor = texture2D(outputImgTex, uv);
    return;
  }

  gl_FragColor = bilerpColor(outputImgTex, p, r);
}
